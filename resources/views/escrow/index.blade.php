            ...
            <div class="border-b border-gray-200 dark:border-dark-700">
                <nav class="flex -mb-px">
                    <a href="{{ route('wallet.escrow') }}" class="py-4 px-6 text-center border-b-2 font-medium text-sm 
                        {{ request()->routeIs('wallet.escrow') && !request()->get('status') ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300' }}">
                        All
                    </a>
                    <a href="{{ route('wallet.escrow.active') }}" class="py-4 px-6 text-center border-b-2 font-medium text-sm 
                        {{ request()->routeIs('wallet.escrow.active') ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300' }}">
                        <i class="fas fa-lock mr-2"></i>Active
                    </a>
                    <a href="{{ route('wallet.escrow.released') }}" class="py-4 px-6 text-center border-b-2 font-medium text-sm 
                        {{ request()->routeIs('wallet.escrow.released') ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300' }}">
                        <i class="fas fa-check mr-2"></i>Released
                    </a>
                    <a href="{{ route('wallet.escrow.disputed') }}" class="py-4 px-6 text-center border-b-2 font-medium text-sm 
                        {{ request()->routeIs('wallet.escrow.disputed') ? 'border-red-500 text-red-600 dark:text-red-400' : 'border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300' }}">
                        ...
                        <div class="p-6">
                            <div class="flex items-center justify-between">
                                <div class="flex-1">
                                    <div class="flex items-center gap-3 mb-2">
                                        <h3 class="font-semibold text-gray-900 dark:text-gray-100">
                                            @if($escrow->task)
                                                {{ $escrow->task->title }}
                                            @elseif($escrow->service)
                                                {{ $escrow->service->title }}
                                            @elseif($escrow->growthListing)
                                                {{ $escrow->growthListing->title }}
                                            @elseif($escrow->digitalProduct)
                                                {{ $escrow->digitalProduct->title }}
                                            @else
                                                Escrow Transaction #{{ $escrow->id }}
                                            @endif
                                        </h3>
                                        <span class="px-3 py-1 text-xs font-medium rounded-full 
                                            @if($escrow->status === 'held') bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400
                                    ...
                                    <div class="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
                                        <span>
                                            ...
                                            @if($escrow->buyer_id === Auth::id())
                                                To: {{ $escrow->seller->name }}
                                            @else
                                                From: {{ $escrow->buyer->name }}
                                            @endif
                                        </span>
                                ...
                                <div class="text-right">
                                    ...
                                    @if($escrow->status === 'held' && $escrow->buyer_id === Auth::id())
                                        <form action="{{ route('wallet.escrow.release', $escrow) }}" method="POST" class="mt-2">
                                            @csrf
